...
consider a constant $(Y \,=\, 0)$ model. It is important to note that this hypothesis test tells us
that at least one parameter is non-zero, but it doesn't tell us **which**.
...
could be read off the coefficient summary table.
...
> The $p$-value for the test of `var2` different from zero is larger than 0.05 so we fail to reject
the null hypothesis. The $p$-value for the test of `var3` different from zero is larger than 0.05 so
we fail to reject the null hypothesis. Therefore we can simultaneously remove `var2` and `var3` from
our model.
...
using $t$-tests would be to test for a single parameter non-zero, remove the variable from the model
(assuming we failed to reject the null hypothesis), refit a new model without this variable, and
then perform our second $t$-test.
