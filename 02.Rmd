...
I have added some additional arguments to `geom_line()` outside of the aesthetics since these values
...
highway miles per gallon when the city miles per gallon is 20. We can obtain these values by using
the `predict()` function. More specifically, when we pass our `lm` object to predict, it actually
calls the more specific `predict.lm()` function. To see what we need to get our point estimate
and confidence interval, let's pull up the associated documentation.
