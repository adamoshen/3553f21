...
is automatically set to the location where the R script file is located. 
...
2. For users that are not on Windows, in addition to the folders possibly not existing, you likely
...
To check if the normal distribution assumption has been satisfied, the plotted points should lie as
close as possible to the QQ-line. As this is the case in the above plot, we have no reason to
believe that the residuals are *not* normally distributed.
