...
The second argument to `add1()` is the scope, where we list all predictors (using formula syntax)
that are eligible to be added to the main model. Notice that the results of the partial $F$-tests
are identical to when we had constructed the models individually before (`step2a` and `step2b`).

The variable that gets added to our model is the one with the highest $F$-value. Since each of these
$F$-values have the same numerator and denominator degrees of freedom, this is also equivalent to
adding the variable that has the smallest $p$-value associated to its partial $F$-test (so long as
it is less than $\alpha_{\text{entry}}$). Since the $F$-test of adding `bill_length` to the base
model results in the smallest $p$-value that is less than $\alpha_{\text{entry}}$, `bill_length`
...
The second argument to `drop1()` is the scope (similar to `add1()`), where we list all predictors
(using formula syntax) that are eligible to be dropped from the main model. Here, all predictors
currently in the model are eligible to be dropped.

Recall the hypotheses associated to dropping a single variable:

\[H_{0}: \beta_{1} = 0, \quad H_{A}: \beta_{1} \neq 0\]
\[H_{0}: \beta_{2} = 0, \quad H_{A}: \beta_{2} \neq 0\]

For a variable to be dropped from the model, we need to fail to reject the null hypothesis. So we
are looking for small $F$-values, or equivalently, large $p$-values that are greater than
$\alpha_{\text{exit}}$. Since we do not have any here, we do not drop any of the predictors in the
current model.
...
model that contained all predictors, none were eligible for dropping. For this example, let us
consider the 
...
For this example, we will only keep a subset of the variables. In addition, some of the variables
that should be of type numeric were read in as a strings due to the presence of the string `"NULL"`
(note that the string `"NULL"` is not the same as `NA`).
...
Now we are ready to start building models. We are interested in a creating a model that predicts
the IMDB score. As we are performing backward selection, we start with the model containing all
...
be dropped. Therefore, we look for the predictor corresponding to the largest $p$-value greater than
...
Interestingly, this says that the number of jinkies in an episode will negatively impact your IMDB
score &#x1F602;!

Note also, that while all the remaining predictors in our model are significant, we have a very
poor adjusted R-squared value of 0.3336. This means that we have only explained 33.36% of the
variation in IMDB ratings by using these predictors. As such, this model still has room for
improvement (though do keep in mind that only a small subset of variables were included in
